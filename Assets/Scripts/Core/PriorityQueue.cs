...
namespace Pathfinding
{
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
		...
		public void UpdateItem (TValue value)
		{
		...
		public int IndexOf (TValue item)
		{
			...
			{
				if (_baseHeap[i].Value.Equals (item)) return i;
			}
	...
	public class PriorityQueue2<TPriority, TValue>
	{
		private SortedDictionary<TPriority, LinkedList<TValue>> list = new SortedDictionary<TPriority, LinkedList<TValue>>();
		...
		public TValue Dequeue ()
		{
			...
			SortedDictionary<TPriority, LinkedList<TValue>>.KeyCollection.Enumerator enume = list.Keys.GetEnumerator();
			enume.MoveNext();
			TPriority key = enume.Current;
			LinkedList<TValue> v = list[key];
			TValue res = v.First.Value;
			v.RemoveFirst();
			if (v.Count == 0)
			{ 
		...
		public void Replace (TValue oldValue, TValue newValue, TPriority oldPriority, TPriority newPriority)
		{
			LinkedList<TValue> v = list[oldPriority];
			v.Remove (oldValue);
		...
		{
			string res = "";
			foreach (TPriority key in list.Keys)
			{
				foreach (TValue val in list[key])
				{
					res += val +", ";
				}
			}
